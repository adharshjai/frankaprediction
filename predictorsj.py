# predictorsj.py (Final Hybrid Model Version)
...
def generate_ground_truth_labels(num_labels):
    """Generates the theoretical direction and speed labels for the detected movements."""
    timerise, timehold, omega_max = 0.5, 2.0, 0.6
    T = 2 * timerise + timehold
    labels, cache = [], {}
    for i in range(num_labels):
        current_time = i * T
        cycle = int(current_time // T)
        direction = 0 if cycle % 2 == 0 else 1
        speed = omega_max * _randnumgen(cycle // 2, cache)
        labels.append({'direction': direction, 'speed': speed})
    return labels


def augment_audio_data(clips, scalograms, directions, speeds, target_size=150):
    """Augment the small dataset to make it larger for training."""
    print(f"Augmenting {len(clips)} samples to {target_size} samples...")

    augmented_clips = list(clips)
    augmented_scalograms = list(scalograms)
    augmented_directions = list(directions)
    augmented_speeds = list(speeds)

    while len(augmented_clips) < target_size:
        idx = np.random.randint(0, len(clips))
        original_clip = clips[idx]

        # Audio augmentation: Add noise
        noise_factor = np.random.uniform(0.001, 0.005)
        augmented_clip = original_clip + noise_factor * np.random.normal(size=original_clip.shape)
        augmented_clips.append(augmented_clip.astype(np.float32))

        # For simplicity, we reuse the original scalogram for the augmented audio
        augmented_scalograms.append(scalograms[idx])
        augmented_directions.append(directions[idx])
        augmented_speeds.append(speeds[idx])

    print(f"Augmentation complete: {len(augmented_clips)} total samples")
    return augmented_clips, np.array(augmented_scalograms), np.array(augmented_directions), np.array(augmented_speeds)
...
def build_hybrid_model(input_shape_audio, input_shape_scalogram):
    """
    A model that uses a more powerful audio branch (LSTM) for regression and a
    simple image branch for classification.
    """
    print(f"Building HYBRID model...")

    # Audio branch - Restore the LSTM to capture temporal patterns for speed
    ...
    x1 = layers.MaxPooling1D(4)(x1)
    # The LSTM is much better at finding patterns in time than GlobalMaxPooling
    x1 = layers.Bidirectional(layers.LSTM(32, return_sequences=False))(x1)
    x1 = layers.Dropout(0.4)(x1)

    # Scalogram branch - Keep it simple
...
if __name__ == "__main__":
    run_folder = "robot_movements_dataset/run_2025-09-09_22-22-55"
    ...
    if not run_folder:
        ...
        if data:
            ...
            # Augment data if the dataset is very small
            if len(clips) < 100:
            ...
            # --- Normalize the speed values (target labels) ---
